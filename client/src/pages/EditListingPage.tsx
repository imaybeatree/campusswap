...
export default function EditListingPage() {
    ...
    http()
      ...
      .then((res) => {
        ...
        if (l.image_url) {
          ...
          setImagePreview(imageUrl(l.image_url));
        }
      })
  ...
  const handleSubmit = async (e: React.SubmitEvent) => {
    ...
    setSaving(true);
    try {
      await http().put(`/api/listings/${id}`, {
        ...form,
    ...
    <div className="min-h-screen bg-background">
      <nav className="border-b border-border px-6 py-4">
        <div className="max-w-2xl mx-auto">
          <Link to={`/listings/${id}`} className="text-sm font-medium hover:underline">&larr; Back to listing</Link>
        </div>
      </nav>
      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-3">
            ...
            <Button type="button" variant="outline" className="cursor-pointer" onClick={() => navigate(`/listings/${id}`)}>
              Cancel
            </Button>
          </div>
        </form>
